...
R: Não. Pois assim a thread 'B' verificaria apenas uma vez a condição de ser múltipo de 10. Desse modo, ela poderia imprimir um valor que não seja múltiplo de 10 pela condição de corrida.
...
R: Na maior parte da execução sim, porém teve um caso que evidenciou a condição de corrida imanente:
    $ ./printX.out 
    A: Comecei: thread 1
    B: Comecei: thread 3
    A: Comecei: thread 2
    A: thread 1, x = 10, encontrou padrao 
    B: thread 3 x = 16, vai se bloquear...
    A: thread 1, x = 20, encontrou padrao 
    B: thread 3, sinal recebido e mutex realocado. x = 23
    X=23
